<x-admin-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <x-admin.card title="Edit Navigasi">
                <form action="{{ route('admin.navigations.update', $navigation) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <x-admin.input type="text" name="name" id="name" class="mt-1 block w-full" :value="old('name', $navigation->name)" required autofocus />
                    ...
                    <div>
                        ...
                        <select name="parent_id" id="parent_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            @foreach(\App\Models\Navigation::whereNull('parent_id')->where('id', '!=', $navigation->id)->orderBy('order')->get() as $parent)
                    ...
                    <div>
                        ...
                        <select name="news_id" id="news_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            @foreach(\App\Models\News::where('is_active', true)->orderBy('title')->get() as $news)
                                <option value="{{ $news->id }}" {{ old('news_id', $navigation->news_id ?? '') == $news->id ? 'selected' : '' }}>
                                    {{ $news->title }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <x-admin.input type="text" name="icon" id="icon" class="mt-1 block w-full" :value="old('icon', $navigation->icon)" />
                    ...
                    <div>
                        ...
                        <x-admin.input type="number" name="order" id="order" class="mt-1 block w-full" :value="old('order', $navigation->order)" min="0" required />
                        <x-admin.input-error for="order" class="mt-2" />
                    </div>

                    <div class="flex items-center">
                        <x-admin.checkbox id="active" name="active" :checked="old('active', $navigation->active)" />
