<x-admin-layout>
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <x-admin.card title="Tambah Infografis">
                <form action="{{ route('admin.infographics.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <select name="infographic_type_id" id="infographic_type_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            @foreach(\App\Models\InfographicType::where('is_active', true)->orderBy('order')->get() as $type)
                                <option value="{{ $type->id }}" {{ old('infographic_type_id') == $type->id ? 'selected' : '' }}>
                                    {{ $type->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <x-admin.textarea name="data" id="data" class="mt-1 block w-full h-64 font-mono">{{ old('data', '[]') }}</x-admin.textarea>
                    ...
                    <div>
                        ...
                        <select name="diagram_type" id="diagram_type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            <option value="bar" {{ old('diagram_type') == 'bar' ? 'selected' : '' }}>Bar Chart</option>
                            <option value="pie" {{ old('diagram_type') == 'pie' ? 'selected' : '' }}>Pie Chart</option>
                            <option value="line" {{ old('diagram_type') == 'line' ? 'selected' : '' }}>Line Chart</option>
                            <option value="table" {{ old('diagram_type') == 'table' ? 'selected' : '' }}>Table</option>
                        </select>
